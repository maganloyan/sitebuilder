...
# @frappe.whitelist()
# def generate_front_setup_files():
#     # Fetch all entries from Front Setup Item where generate is 1
#     front_setup_items =frappe.get_doc("Site Settings").as_dict().front_setup
    
    
#     for item in front_setup_items:
...
#         if not app_name or not app_path or not title or not extension or not template:
#             frappe.throw(_("All fields (app_name, app_path, title, extension, template) must be provided."))

#         # Create the full app path using Path object for platform independence
#         full_app_path = Path("../apps") / app_name / app_path
#         full_file_path = full_app_path / f"{title}.{extension}"

#         # Ensure the directory exists, create it if not
#         full_app_path.mkdir(parents=True, exist_ok=True)
...
#                     f.write(template)
#                     frappe.msgprint(_(f"File {full_file_path} created successfully."))
#             except Exception as e:
#                 frappe.throw(_("Error writing to file: {0}").format(str(e)))


# @frappe.whitelist()
# def generate_single_front_file(docname):
#     # Fetch the specific Front File document by name
#     front_file = frappe.get_doc("Front File", docname)
    
#     # Extract necessary fields
#     app_name = front_file.app_name
#     app_path = front_file.app_path
#     label = front_file.label
#     extension = front_file.extension
#     code = front_file.code

#     # Ensure all required fields are provided
#     if not app_name or not app_path or not label or not extension or not code:
#         frappe.throw(_("All fields (app_name, app_path, label, extension, code) must be provided."))

#     # Create the full app path using Path object for platform independence
#     full_app_path = Path("../apps") / app_name / app_path
#     full_file_path = full_app_path / f"{label}.{extension}"
...
# @frappe.whitelist()
# def generate_stepone_front_files():
#     # Fetch all entries from Front File where generate is 1
#     front_files = frappe.get_all("Front File", filters={"step": "1"}, fields=["name"])

#     for item in front_files:
#         # Call the function to generate each file by passing the docname
#         generate_single_front_file(item["name"])


# @frappe.whitelist()
# def generate_steptwo_front_files():
#     # Fetch all entries from Front File where generate is 1
#     front_files = frappe.get_all("Front File", filters={"step": "2"}, fields=["name"])
...
#     # Fetch Block Generation Settings document
#     block_generation_settings = frappe.get_doc("Web Settings").as_dict().block_settings

#     # Iterate through all block settings to find a valid app
#     for setting in block_generation_settings:
#         app_name = setting.app_name
#         app_path = Path("../apps") / app_name
#         components_directory = app_path / setting.app_path / folder_name
