...
class SiteBlock(Document):
	...
	if TYPE_CHECKING:
		...
		from sitebuilder.sitebuilder.doctype.site_block_field.site_block_field import SiteBlockField

		fields: DF.Table[SiteBlockField]
		module: DF.Link | None
		standard: DF.Check
	...
	def on_update(self):
		...
		routes = frappe.get_all(
			"Site Page",
			filters=[
				["Site Page Item", "web_template", "=", self.name],
				["Site Page", "published", "=", 1],
			],
			pluck="route",
		)
		for route in routes:
	...
	def get_template_folder(self):
		...
		module = self.module or "fagaaraha"
